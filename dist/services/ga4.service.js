...
exports.ga4Service = exports.GA4Service = void 0;
const googleapis_1 = require("googleapis");
const google_service_1 = require("./google.service");
const integrations_service_1 = require("./integrations.service");
class GA4Service {
    async getClient() {
        const creds = await integrations_service_1.integrationsService.getSecureCredentials('ga4');
        ...
        onTokens(async (tokens) => {
            ...
            if (tokens.access_token) {
                ...
                await integrations_service_1.integrationsService.saveCredentials('ga4', tokens.access_token, newRefreshToken, creds.config);
            }
        });
        return client;
    }
    async getProperties() {
        ...
        const adminApi = googleapis_1.google.analyticsadmin({ version: 'v1beta', auth: client });
    ...
    async setPropertyId(propertyId) {
        ...
        await integrations_service_1.integrationsService.saveCredentials('ga4', creds.accessToken, creds.refreshToken || null, config);
    }
    async runReport(dimensions, metrics, startDate = '30daysAgo', endDate = 'today') {
        const creds = await integrations_service_1.integrationsService.getSecureCredentials('ga4');
        if (!creds?.accessToken)
            return null;
        const config = creds.config;
        if (!config?.propertyId)
            return null;
        const client = await this.getClient();
        const dataApi = googleapis_1.google.analyticsdata({ version: 'v1beta', auth: client });
        try {
...
exports.ga4Service = new GA4Service();
