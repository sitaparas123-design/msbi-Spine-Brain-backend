...
function isZodDate(entity) {
	return entity instanceof zod_v4_core.$ZodType && entity._zod.def.type === "date";
}
function isZodUnion(entity) {
	return entity instanceof zod_v4_core.$ZodType && entity._zod.def.type === "union";
}
function isZodUndefined(entity) {
	return entity instanceof zod_v4_core.$ZodType && entity._zod.def.type === "undefined";
}
/**
...
var zodSchemaToJson = (zodSchema, registry, io, config) => {
	...
	*/
	...
	if (schemaRegistryEntry?.id) return { $ref: require_utils.getReferenceUri(schemaRegistryEntry.id) };
	/**
	...
	*/
	const tempRegistry = new zod_v4_core.$ZodRegistry();
	tempRegistry.add(zodSchema, { id: SCHEMA_REGISTRY_ID_PLACEHOLDER });
	const { schemas: { [SCHEMA_REGISTRY_ID_PLACEHOLDER]: result } } = (0, zod_v4_core.toJSONSchema)(tempRegistry, {
		...config,
		io,
		metadata: registry,
	...
	*/
	return JSON.parse(JSON.stringify(sanitized), (_key, value) => typeof value === "string" && value.startsWith(SCHEMA_URI_PLACEHOLDER) ? require_utils.getReferenceUri(value.slice(23)) : value);
};
var zodRegistryToJson = (registry, io, config) => {
	const result = (0, zod_v4_core.toJSONSchema)(registry, {
		...config,
		io,
		metadata: registry,
		unrepresentable: config.unrepresentable ?? "any",
		cycles: "ref",
		reused: "inline",
		uri: (id) => require_utils.getReferenceUri(id),
		override: config.override ?? ((ctx) => getOverride(ctx, io, config.target))
	}).schemas;
